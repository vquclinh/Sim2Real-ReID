...
Full architecture documentation for the pipeline for **AI City Challenge 2026 — Track 4: Text-Based Person Anomaly Search**.

> This document is written so that a newcomer to the project can understand: (1) the problem being solved, (2) the foundational algorithm from the AIO paper (WWW 2025), (3) the 4 upgrades we apply, (4) the details of each notebook in the pipeline, (5) the Colab/Drive/Kaggle infrastructure, and (6) bugs that have been fixed.

---

## Table of Contents

1. [Problem statement and metric](#1-problem-statement-and-metric)
...
6. [Adaptive Ensemble — Detailed Math](#6-adaptive-ensemble--detailed-math)
7. [Infrastructure (`aic_colab_utils.py`)](#7-infrastructure-aic_colab_utilspy)
8. [Validation strategy](#8-validation-strategy)
9. [Critical bugs fixed](#9-critical-bugs-fixed)
...
**Text-Based Person Anomaly Search**: given a query text describing a person's behavior (which may be normal or anomalous), find and rank the top-10 images from a gallery of 36,773 real-world images containing the person matching that description.

### Dataset — PAB (Pedestrian Anomaly Behavior) ECCV 2026

| Split | Count | Characteristics |
|-------|---------|----------|
...
**Core difficulty (Sim2Real gap):** the model must learn from synthetic images but perform inference on real images → significant distribution shift.

### Metric

- **Primary**: mAP (mean Average Precision) over 1,978 queries × 36,773 gallery
- **Secondary**: R@1, R@5, R@10
- **Submission format**: `answer.txt` with 1,978 lines, each line containing 10 gallery IDs space-separated
- **Single-positive setting**: each query has **1 GT image** in the gallery → AP = `1/(rank+1)` if rank<10 else 0
...
Reference: **"Hybrid, Unified and Iterative: A Novel Framework for Text-based Person Anomaly Retrieval"** (Tien-Huy Nguyen et al., ACM WWW 2025) — achieves R@1 = 89.23% on PAB.

The AIO pipeline has **3 main components**:

### 2.1 LHP — Local-global Hybrid Perspective

**Purpose**: the model learns both fine-grained details (local view) and holistic context (global view) instead of just one perspective.

**Probabilistic sampling**: each training sample randomly selects a local or global view:
...
**Backbone**: BEiT-3-large @ 384×384, 3 epochs, batch 384.
...
**Purpose**: learn a joint image-text representation through 4 simultaneous tasks (multi-task learning).
...
**Training**: AdamW lr=1e-5, wd=0.05, cosine LR + warmup 5%, batch 84, 224×224, 22 epochs.

### 2.3 Algorithm 1 — Feature Selection (LHP → UIT)

**Purpose**: bridge LHP and UIT — LHP acts as a "guide" that selects candidates for the UIT cross-encoder.
...
→ This is the **killer flaw if ITM is dropped**: using only cosine ITC discards 1/4 of the training signal.

### 2.4 Algorithm 2 — Iterative Ensemble

**Purpose**: combine 3 models sequentially, with each round "gently pulling" the ranking toward the new model.

**Paper-original (3 rounds)**:
...
Fixed weights, manually tuned by grid search.
...
| # | Upgrade | Rationale |
|---|---------|-------|
| **1** | **Add PE-Core-G14-448** (Facebook Perception Encoder G14) as 4th model in the ensemble | PE-G14 1.8B params, state-of-the-art zero-shot CLIP-style, strongest standalone model |
| **2** | **Extend iterative loop → 4 rounds** with PE-G14 as base (reversed order: strongest first) | Paper order (UIT base) causes UIT to occupy ~72% effective contribution even though it is not the strongest model |
| **3** | **Replace BEiT-3 → PE-G14-448 in LHP**, fine-tuned with **LoRA r=16** | PE-G14 (1.8B) full fine-tuning would catastrophically forget zero-shot knowledge on 1M synthetic samples. LoRA freezes 100% of the backbone and trains only ~8-12M params (~0.5%) on Q/K/V projections of MHA |
| **4** | **Adaptive Weighting** replaces fixed weights | Paper weights $w \in \{0.9125, 0.925, 0.85\}$ are manually tuned and do not account for per-query confidence variability |

**Extra upgrade**: k-reciprocal Jaccard re-ranking on each model before fusion (+2-3 mAP from literature).
...
└─────────────────────────────────┬────────────────────────────────────┘
  ...
  - gallery, queries        - keypoints for LHP
...
|---|----------|-------|-----------------|
| 00 | `00_manifest_qc.ipynb` | Build manifests + val splits (run on Kaggle or Colab) | ~30 min |
| 01a | `01a_pe_g14_features.ipynb` | PE-G14 encode (gallery + queries + val) | ~25-75 min |
| 01b | `01b_vitpose_features.ipynb` | ViTPose keypoints (**OPTIONAL** — only for LHP anatomical crop) | ~3h |
| 02 | `02_uit_train.ipynb` | UIT training (Swin-B + BERT, 22 epochs) | ~8h |
| 03 | `03_lhp_peg14_train.ipynb` | LHP with PE-G14 + LoRA | ~2-3h |
...
| 07 | `07_pe_g14_scores.ipynb` | Build Q×G score matrix from 01a embeddings | ~5 min |
...
**Total wall-clock** (skip 01b ViTPose, use 01a partial): ~14-16h on A100 80GB. Can be split across multiple Colab sessions with resume from Drive.
...
**Purpose**: parse PAB JSONL annotations + index image files → build canonical parquet manifests + create val splits for gating.
...
- `train_manifest_trainable.parquet` (~95% = ~962K rows, used to train UIT + LHP)
...
**Note**: the notebook works on both Kaggle (`/kaggle/input/` → `/kaggle/working/`) and Colab (via `aic_colab_utils`).
...
CHANNELS_LAST = True
USE_COMPILE = False           # ← DISABLED due to CUDA Graphs bug with PE-G14 RoPE cache

# Run flags (default optimized for pipeline)
RUN_TRAIN_IMAGE_EMBEDDINGS = False   # ← train embeds not consumed by anyone, saves 5h
RUN_TRAIN_TEXT_EMBEDDINGS = False
RUN_GALLERY_IMAGE_EMBEDDINGS = True  # ← required for 07 (Round 4 ensemble)
RUN_QUERY_TEXT_EMBEDDINGS = True
RUN_VAL_IMAGE_EMBEDDINGS = True      # ← required for 03 fallback compare
...
**Resumability**: `chunk_done()` checks local SSD + Drive before encoding. Async sync per chunk → Drive.
...
**Pipeline**:
1. RTDetr detects persons in each image (top-1 per image, score × area ranking)
2. Crop person bbox → ViTPose++ infers 17 COCO keypoints (x, y, confidence)
3. Normalize keypoints to [0,1] image space

**Output**: `pose/vitposepp/train/chunk_*.npz` + `pose/vitposepp/gallery.npz` with keypoints (17, 3) per image.

**Role in pipeline**: only used for **LHP anatomical crop** in `03` (Upgrade enhancement, not in the original paper — the paper uses random crop).

**Skippable**: the pipeline has a fallback guard — if the pose dict is empty, LHP local view will use random crop instead of anatomical crop (paper-faithful). Skipping saves ~3h.
...
# (Drive resume logic: latest checkpoint_<n>.pth restored at Step 1)
```
...
### 5.5 `03_lhp_peg14_train.ipynb` — LHP with PE-G14 LoRA (Upgrade 3)
...
| Fine-tune | Full | **LoRA r=16 on Q/K/V** |
...
**Why LoRA**: PE-G14 1.8B full FT on 1M synthetic captions would catastrophically forget zero-shot pretraining → loses generalization to real-world test. LoRA trains only ~8-12M params, preserving alignment.
...
# CRITICAL: proj_head MUST be applied at BOTH training AND inference (see Fix 1, §9)
...
# Step 5: ITC training loop (3 epochs)
...
if best_val_mAP10 < zero_shot_PE_G14_val_mAP10:
    use_lora = False   # → 04 uses vanilla PE-G14 for LHP guidance
```
...
- `features/lhp_peg14/scores_lhp.pt` ← **input for Algorithm 1 in 04**

---

### 5.6 `04_uit_inference.ipynb` — Algorithm 1 (LHP-guided ITM Rerank)

**THE most important inference notebook**. Implements Algorithm 1 in full — skipping this step means discarding 1/4 of the training signal (ITM loss).
...
# → for each query, get top-256 candidates proposed by LHP

# Step 6: ITM rerank on top-K candidates
...
for q in range(len(q_ids)):
    ...
    # Convex combine: final = α · ITC + (1-α) · ITM for top-K positions
...
**Critical**: UIT's cross-encoder ITM only runs on **top-K=256 candidates selected by LHP** (Algorithm 1). Running ITM on all 36,773 gallery entries would be prohibitively expensive (1978 × 36773 = 73M cross-encoder calls). Top-K=256 → 506K calls, ~30 min.

**Outputs**:
- `features/uit/{gallery,queries,val,val_zs}.npz` (ITC pooled embeds)
- `features/uit/scores_uit.pt` ← **final Round 1 score** (Algorithm 1 applied)
- `features/uit/scores_uit_itc_only.pt` (ablation: ITC only, without Algorithm 1)

---

### 5.7 `05_blip2_inference.ipynb` — BLIP-2 ITC + ITM Rerank (Round 2)

**Model**: LAVIS `blip2_image_text_matching` (Salesforce BLIP-2 with ViT-g vision tower + Q-Former).

**Pipeline** similar to `04` but uses BLIP-2's own ITC for top-K selection (not LHP):
...
# 2. ITM rerank top-K=1024 (BLIP-2 ITM is lighter than UIT, larger K is feasible)
ITM_TOPK = 1024
ITC_ALPHA = 0.4
...
**Model**: `openai/clip-vit-large-patch14-336` (NOT EVA-CLIP — paper-faithful uses OpenAI weights).
...
Tiny notebook — only builds the score matrix from embeddings already computed in `01a`:
...
**ETA**: ~5 min (only matmul 1978×1280 @ 1280×36773 fp32 = 1 GFLOP).

---

### 5.10 `08_kreciprocal_rerank.ipynb` — k-Reciprocal Re-ranking (Extra Upgrade)

**Algorithm** (Zhong et al. CVPR 2017, adapted for text→image cross-modal):
...
**Outputs**: `features/<m>/scores_<m>_rr.pt` for each m ∈ {uit, blip2, clip, pe_g14}.

**Why before fusion**: k-reciprocal improves per-model score distribution → adaptive confidence in `09` is more accurate.

---

### 5.11 `09_adaptive_ensemble_submit.ipynb` — Adaptive 4-way + Submission (Upgrades 2 + 4)

The most important notebook — contains both final upgrades + generates `answer.zip`.

See [§6 Adaptive Ensemble Math](#6-adaptive-ensemble--detailed-math) to understand the algorithm.
...
chosen = argmax_with_threshold(metrics)   # auto-fallback if adaptive does not beat fixed
...
**Outputs**:
- `submission/answer.txt` (1978 lines × 10 gallery IDs)
...
- `validation/ablation_table.parquet` (mAP@10/R@1 for each config)
- `validation/iter_weight_grid.parquet` (top weight configs from grid search)

---

## 6. Adaptive Ensemble — Detailed Math
...
where $\mu_m(q) = \frac{1}{|G|} \sum_g s_m(q, g)$, $\sigma_m(q) = \text{std}_g(s_m(q, g))$.

**Why this is critical**: BLIP-2 ITM outputs cross-entropy logits with a range completely different from CLIP/PE-G14 cosine $\in [-1, 1]$. Without z-norm → the model with the largest scale will dominate fusion regardless of weight.

### 6.2 4 confidence signals

For each model $m$ and query $q$, take top-K=20 similarity values $s^{(m)}_{(1)}(q) \geq s^{(m)}_{(2)}(q) \geq \dots \geq s^{(m)}_{(K)}(q)$:
...
**d) Inter-model agreement** (fraction of top-K overlap with the other 3 models):
```math
...
Defaults after val tuning: $\alpha = 1.0$, $\beta = 1.0$, $\gamma = 0.5$, $\lambda = 0.5$.
...
with $M = 4$, $\eta = 0.05$ (floor — no model is completely disabled), $T = 1.0$ (softmax temperature).
...
with $w_2 = 0.85$, $w_3 = 0.925$, $w_4 = 0.9125$. Effective contribution after 3 rounds:
...
On the held-out val set (val_split + val_zeroshot_scene), evaluate 5 configs:
...
**Selection metric**: mAP@10. **Threshold**: only promote if it beats the current chosen config by ≥0.5%. Default winner: `iter4_reversed`.

---

## 7. Infrastructure (`aic_colab_utils.py`)

Shared module across all notebooks, providing:
...
2. **Load Kaggle credentials** (`kaggle.json` from Drive or arg)
3. **Restore raw + manifests from Drive → local SSD**:
   - Prefer `restore_raw_from_tar_split()` (~30-40 min for 100GB) if Drive has `raw_tar_parts/`
   - Fallback rsync many-files (legacy, ~30-60 min)
4. **Download missing datasets** via `kagglehub.dataset_download()` (only when `.synced_*` marker is absent)
5. **Resume output chunks** from Drive (incremental)
6. **Resolve concrete subdirs** (`annotation_train_dir`, `test_dir`, `gallery_dir`) — auto-detect both with and without `PAB/` prefix

**Returns** `paths` dict with 9 keys: `drive_root`, `local_root`, `input_root`, `manifest_dir`, `output_root`, `drive_output_root`, `annotation_train_dir`, `test_dir`, `gallery_dir`.

### 7.2 Drive persistence — 2 strategies

**Tar-split** (recommended for 100GB raw):
```python
mirror_raw_as_tar_split(paths, part_size='4500M')
# tar -chf - -C local_root raw | split -b 4500M - drive/raw_tar_parts/raw.tar.part_
# -h flag DEREFERENCES symlinks (important if raw is a symlink farm pointing to kagglehub cache)
```

**Rsync** (legacy, 3-10h for many-small-files):
```python
...
wait_for_pending_syncs(timeout=600)   # at end of notebook to flush
```
...
# True if chunk already exists on local SSD OR Drive (auto-restore from Drive if needed)
...
# Auto-pick A100/H100 if available
...
# 3 strategies per attempt:
#   1. Python pathlib.mkdir(parents=True, exist_ok=True)
#   2. Shell `mkdir -p` via subprocess (bypass FUSE quirks)
#   3. Re-check exists() — FUSE sometimes raises but the folder was still created
...
| `train_manifest_trainable` | remaining ~90% | — | Train UIT + LHP |

### 8.2 Metric — mAP@10 (single-positive)

**Do NOT use full mAP**. AIC submission requires top-10 → val gate must match:
...
All training notebooks (02 UIT, 03 LHP) + ensemble (09):
- Compute mAP@10 on val
- Compare with baseline (zero-shot model OR fixed-weight ensemble)
- Auto-revert if trained version < baseline - 0.5%
- Record decision in JSON: `fallback_decision.json` / `adaptive_weights_meta.json`

---

## 9. Critical bugs fixed

### 9.1 Bug 1 — `proj_head` orphan in `03_lhp_peg14_train.ipynb`

**Before fix**: training computed `proj_head(encode_image(x))` for the ITC loss, but val eval and final inference both **ignored proj_head** and used raw 1280-d features. LoRA weights were optimized to work with proj_head → became orphaned.

**Fix**: pipe through proj_head in all 3 places (train, val eval, inference). The fallback case (LoRA loses) sets `proj_head = None` to use raw 1280-d zero-shot PE-G14.

### 9.2 Bug 2 — Algorithm 1 skipped in `04_uit_inference.ipynb`

**Before fix**: computed `S_uit = Q @ G.T` cosine only — this is ITC only. UIT was trained for 22 epochs with ITC + ITM + MLM + MIM → skipping ITM means discarding 1/4 of the training signal.

**Fix**: correctly implement Algorithm 1:
...
### 9.3 Bug 3 — Iterative ensemble order wrong (PE-G14 only 15%)

**Before fix**: paper order UIT→BLIP-2→CLIP→PE-G14 with weights [0.9125, 0.925, 0.85] caused UIT to occupy 71.7% effective contribution, while PE-G14 (the strongest standalone model) got only 15%.

**Fix**: reverse order to PE-G14→CLIP→BLIP-2→UIT (strongest first). New effective contribution: PE-G14 71.7%, CLIP 12.7%, BLIP-2 6.8%, UIT 8.8%. Bonus: grid search 80 weight configs on val mAP@10.

### 9.4 Bug 4 — Metric mismatch (Recall vs mAP)

**Before fix**: paper optimized R@1, val gate used top-1 accuracy. AIC 2026 evaluates by **mAP@10** — if GT is at rank 10 (Recall@10=1 but AP=0.1) it gets undercounted.

**Fix**: change all val gate criteria → mAP@10. Compute mAP@10, mAP@100, R@1/5/10 simultaneously for diagnostics.

### 9.5 Bug 5 — `torch.compile(mode='reduce-overhead')` crash with PE-G14

**Before fix**: CUDA Graphs reuse memory buffers, PE-G14's `rope.py:338` stores `self.freq = freq[None, ...]` cache → subsequent batches overwrite the buffer → crash with "accessing tensor output of CUDAGraphs that has been overwritten".

**Fix**: `USE_COMPILE = False` by default. PE-G14 on A100 BF16+channels_last+Flash-SDPA is already very fast (~500-1500 img/s) without compilation.

### 9.6 Bug 6 — Drive FUSE `[Errno 5]` Input/output error

**Before fix**: `drive_output.mkdir()` raised OSError randomly due to Drive FUSE state corruption / orphaned folder in trash.

**Fix**: `_robust_drive_mkdir()` with 3 strategies (pathlib mkdir → shell `mkdir -p` → recheck exists) × 3 retries. Setup continues if Drive fails (pipeline still runs locally).

### 9.7 Bug 7 — `tar` archive contains empty symlinks

**Before fix**: `mirror_raw_as_tar_split()` used `tar -cf` to archive symlinks AS symlinks. When raw is a symlink farm pointing to kagglehub cache (due to disk-full workaround), the tar archive contains broken links; restoring in a different session results in empty data.

**Fix**: change `tar -cf` → `tar -chf` (`-h` dereferences symlinks, archives actual content).
...
| `/content/` | ~80 GB | Ephemeral | ~3-5 GB/s | Often pre-filled, ~6-50 GB actually free |
| `/var/colab/` | ~84 GB | Ephemeral | ~3-5 GB/s | Backup target if /content is full |
| `/local-scratch/` | ~370 GB | Ephemeral | ~3-5 GB/s | A100 high-RAM only |
| `/content/drive/` | Drive quota | **Persistent** | **~10-50 MB/s** | FUSE bottleneck with many-small-files |
...
Drive: /content/drive/MyDrive/aic2026_data/raw_tar_parts/  (~22 parts × 4.5GB)
    ↓ next session: cat parts | tar -xf  (~30-40 min)
local SSD restore (no Kaggle DL needed)
```

### 10.3 Symlink workaround (when disk is full)

The pipeline supports the case where `<local_root>/raw/` is a symlink farm pointing to kagglehub cache:
...
Disk usage: ~1MB (symlinks only), instead of duplicating 136GB. `tar -chf` in `mirror_raw_as_tar_split()` dereferences → backup tar contains actual files.
...
```bash
# 1. Upload Kaggle credentials to Drive
#    /content/drive/MyDrive/aic2026_data/.kaggle/kaggle.json

# 2. Upload AIO paper repo to Drive
#    /content/drive/MyDrive/aic2026_data/aio_repo/
#    (clone from paper-released or copy from local workspace)

# 3. Upload pretrained checkpoint (optional, if paper-released)
#    /content/drive/MyDrive/aic2026_data/pretrained/uit_pretrained.pth

# 4. Upload aic_colab_utils.py + notebooks to Colab session
```
...
PATHS = setup_aic2026_environment(
    local_root='/var/colab/aic_local',          # ← largest disk available
...
```
✅ 00 manifest_qc        — run once (on Kaggle or Colab), upload results to Kaggle dataset
✅ 01a PE-G14 partial    — gallery + queries + val (~30 min)
⚠️  01b ViTPose          — OPTIONAL (~3h, only if anatomical crop in 03 is desired)
✅ 02 UIT train          — 22 epochs (~8h, with checkpoint resume from Drive)
✅ 03 LHP-PE-G14 LoRA    — 3 epochs (~2-3h)
...
✅ 07 PE-G14 scores      — build matrix from 01a (~5 min)
✅ 08 k-reciprocal       — rerank per model (~40 min)
✅ 09 adaptive ensemble  — fusion + submission (~15 min)
```

**Total**: ~14-16h on A100-80GB (skip 01b). Split across multiple Colab sessions with Drive resume.

### 11.4 Final submission

`output/submission/answer.zip` contains `answer.txt`:
- 1,978 lines
- Each line: 10 gallery IDs space-separated
- No duplicates per line
- All IDs must be present in `gallery_manifest`
...
Beyond the AIO paper (foundation), the pipeline incorporates algorithms/models from many other sources. The table below provides a full listing and where each appears in the code.

### 12.1 Pretrained models (non-AIO)

| Model | Paper / Source | Role in pipeline | Notebook |
|-------|---------------|------------------------|----------|
...
| **RT-DETR** | Zhao et al., *"DETRs Beat YOLOs on Real-time Object Detection"* (CVPR 2024). HF: `PekingU/rtdetr_r50vd_coco_o365` | Person detection for ViTPose pipeline | 01b |
| **SimMIM** | Xie et al., *"SimMIM: A Simple Framework for Masked Image Modeling"* (CVPR 2022) | MIM loss in UIT (paper-referenced inside CMP class) | 02 (via paper code) |

### 12.2 Algorithms (non-AIO)

| Algorithm | Paper | Role | Notebook | Hyperparams |
|-----------|-------|---------|----------|-------------|
| **LoRA (Low-Rank Adaptation)** | Hu et al., *"LoRA: Low-Rank Adaptation of Large Language Models"* (ICLR 2022). Lib: HuggingFace `peft` | Fine-tune PE-G14 without catastrophic forgetting (Upgrade 3) | 03 | r=16, α=32, dropout=0.05, target=Q/K/V |
| **k-Reciprocal Re-ranking** | Zhong et al., *"Re-ranking Person Re-identification with k-reciprocal Encoding"* (CVPR 2017) | Refine ranking from gallery-gallery k-NN graph before model fusion (Extra Upgrade) | 08 | k1=20, k2=6, λ=0.3, TOPK_RR=200 |
| **InfoNCE / CLIP-style Contrastive Loss** | van den Oord et al., *"Representation Learning with Contrastive Predictive Coding"* (2018); Radford et al. (2021) | ITC loss in UIT + LHP-LoRA training | 02, 03 | τ=0.07 |
| **AdamW optimizer** | Loshchilov & Hutter, *"Decoupled Weight Decay Regularization"* (ICLR 2019) | Optimizer for both UIT + LHP-LoRA | 02, 03 | lr=1e-5 (UIT), 3e-4 (LoRA) |
| **Cosine LR schedule + linear warmup** | Loshchilov & Hutter, *"SGDR: Stochastic Gradient Descent with Warm Restarts"* (ICLR 2017) | LR schedule for training | 02, 03 | warmup 5%, eta_min=1e-6 |
| **Flash Attention 2** | Dao, *"FlashAttention-2: Faster Attention with Better Parallelism and Work Partitioning"* (2023). PyTorch SDPA backend | Speed up attention on A100 | All training/inference notebooks | enabled via `torch.backends.cuda.enable_flash_sdp(True)` |
...
| **Stratified sampling** | Standard statistics | Deterministic val split by `label_type × scene` | 00 | seed=20260514, frac=5% |

### 12.3 Adaptive Ensemble — sources of inspiration

Adaptive Weighting (Upgrade 4) is a **new design of this pipeline**, not copied from any paper, but individual components draw inspiration from:
...
| Paper | Year | Insight applied |
|-------|------|------------------|
| **CUHK-PEDES** (Li et al., CVPR 2017) | 2017 | Text-based person search benchmark establishment |
| **IRRA** (Jiang & Ye, CVPR 2023) | 2023 | SDM (Similarity Distribution Matching) loss — considered but not yet applied |
| **CMP** (ICCV 2025, prior SOTA) | 2025 | 79.33% R@1 on PAB — baseline to beat |
| **HFUT-LMC** (arXiv 2025) | 2025 | SCA (Similarity Coverage Analysis) re-ranking — considered but k-reciprocal used instead |
| **AnomalyLMM** (arXiv 9/2025) | 2025 | Qwen2.5-VL training-free re-ranking — considered for future upgrade A3 |
| **APTM** (ECCV 2024) | 2024 | 6-loss pretraining strategy — referenced in BPAD-E roadmap |
...
| **BPAD-E v2.1 Strategic Roadmap** | Internal team document, [Document/BPAD_E_v2.1_Pipeline.docx](Document/BPAD_E_v2.1_Pipeline.docx) | Roadmap referenced for future upgrades (Query Synthesis, MLLM rerank) |
...
| **OpenCLIP / open_clip_torch** | — | (Was used for EVA-CLIP, no longer needed) |
| **kagglehub** | ≥0.3.0 | Kaggle dataset download (`aic_colab_utils.py`) |
| **timm** | 0.6.13 (paper-pinned for UIT) | Vision transformer utils |
| **torchscale** | 0.3.0 (paper-pinned for UIT) | Microsoft DeepNet utils for UIT |
| **kornia / torchvision** | — | Image transforms |
| **scipy** | — | k-reciprocal Spearman / cosine ops |
| **cupy** (optional) | — | GPU numpy for k-reciprocal |
| **rsync** + **tar** + **split** (Unix) | — | Drive sync infrastructure (`mirror_raw_as_tar_split`) |
| **Google Drive FUSE** (`drivefs`) | — | Persistent storage on Colab |

### 12.7 Summary — Algorithmic provenance map
...
**Summary**: Pipeline foundation from **1 main paper (AIO WWW 2025)**, combined with **~15 other papers** (model architectures + algorithms) + **5-6 newly designed components by the team** (Adaptive Weighting with 4-signal confidence, LoRA-based LHP, mAP@10 gate, Sim2Real val splits, tar-split Drive persistence, symlink workaround).
...
| `notebooks/04_uit_inference.ipynb` | UIT inference with Algorithm 1 |
